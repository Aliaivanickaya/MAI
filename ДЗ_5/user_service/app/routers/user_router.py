...
async def search_users(first_name: Optional[str] = Query(default=None), last_name: Optional[str] = Query(default=None), session: AsyncSession = Depends(get_async_session),):
    """
    Поиск пользователей по имени/фамилии (или только по имени, или только по фамилии).
    """
    users = await user_service.find_users_by_name(session, first_name or "", last_name or "")
